...
var res;
res = swapstore.tryswap("mykey", 9, 0, ["a", "b"], 1);
assert.equal(res.key, "mykey");
assert.equal(res.offset, 2);

res = swapstore.tryswap("mykey", 9, 2, [], 1);
assert.equal(typeof res, "object");
assert.equal(res.key, "mykey");
assert.equal(res.offset, 2);

res = swapstore.tryswap("mykey", 9, 1, [], 1);
assert.equal(res, 404);

res = swapstore.tryswap("mykey", 9, 1, ["b"], 1);
assert.equal(typeof res, "object");
assert.equal(res.offset, 2);

// Doesn't match.
res = swapstore.tryswap("mykey", 9, 1, ["c"], 1);
assert.equal(res, 404);


res = swapstore.tryswap("mykey", 9, 2, ["c", "d"], 1);
assert.equal(res.key, "mykey");
...
res = swapstore.tryswap("mykey", 9, 2, ["c", "d", "e"], 1);
assert.equal(res.key, "mykey");
...
res = swapstore.tryswap("mykey", 9, 5, [], 1);
assert.equal(res.key, "mykey");
assert.equal(res.offset, 5);

res = swapstore.tryswap("mykey", 2, 0, [], 1);
assert.equal(res.key, "mykey");
